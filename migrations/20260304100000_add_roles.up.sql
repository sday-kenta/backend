...
-- Add role_id to users, migrate from is_admin, drop is_admin
ALTER TABLE users ADD COLUMN IF NOT EXISTS role_id INT;

UPDATE users SET role_id = (SELECT id FROM roles WHERE name = 'admin') WHERE is_admin = true;
UPDATE users SET role_id = (SELECT id FROM roles WHERE name = 'user') WHERE is_admin = false OR role_id IS NULL;
